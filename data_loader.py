...
from historical_store import append_market_snapshot, load_market_history
...
def load_scraped_data() -> pd.DataFrame:
    # pull latest live snapshot, then persist it
    snapshot_df = fetch_sharesansar_live_data()
    append_market_snapshot(snapshot_df)
    return snapshot_df
...
def get_latest_market_snapshot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Change" not in latest_df.columns:
        ...
        latest_df["Change"] = latest_df["Change"].fillna(
            latest_df["Close"] - latest_df["Close_Prev"]
        )
